...
// [0] LQR problem
    ...
    std::cout << "\n Q:" << Q;
    std::cout << "\n R:" << R;
    // Mout = solve_continuous_are(Aout,Bout_,Q,R)
    
    Mout_ =  drake::math::ContinuousAlgebraicRiccatiEquation(  Aout, Bout_, Q, R);
...
// Build the system
  ...
  m_=robobeeP.m();
  I_=robobeeP.I();
  auto controller = builder.AddSystem<RobobeeFLController>(m_, I_, Mout_, Bout_);
