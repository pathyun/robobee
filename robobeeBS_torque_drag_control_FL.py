...
Q = 1e0*np.eye(15)
Q[0:3,0:3] = 1e8*np.eye(3) # eta1
Q[3:6,3:6] = 1e3*np.eye(3) # eta2
Q[6:9,6:9] = 1e2*np.eye(3) # eta3
Q[9,9] = 1e2     # eta5
Q[10,10] = 1e0     # eta6
Q[11:14,11:14] = 1e0*np.eye(3) #eta 4
Q[14,14] = 1e0     # eta7
...
def test_Feedback_Linearization_controller_BS(x,t):
    ...
    eta6 = np.dot(alpha,np.dot(Rq,we1))
    print("eta6-eta6_we1 :", eta6-eta6_we1)

    # Second derivative of first three output
    eta3 = np.zeros(3)
    eta3 = -g*e3+Rqe3*xi1+fdw/m - np.array([ddx_f,ddy_f,0])
    ddy1 = eta3[0]
    ddy2 = eta3[1]
    ddy3 = eta3[2]
    ...
    # B_qw = np.dot(Rq,wwe3*xi1+wde3*xi1+2*we3*xi2)+ddfdw_bar_star*bw/m - np.array([ddddx_f,ddddy_f,0]) 
