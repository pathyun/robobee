...
// plants : (i) A GenericRobobee created from the RobobeePlant, and
...
GTEST_TEST(RobobeePlantTest, DirectFeedthrough) {
  RobobeePlant<double> robobee;
  EXPECT_FALSE(robobee.HasAnyDirectFeedthrough());
}
...
// A Generic robobee Plant Diagram with the plant created from
// RobobeePlant.
template<typename T>
class GenericRobobee: public systems::Diagram<T> {
 public:
  GenericRobobee() {
    this->set_name("RobobeeTest");

    systems::DiagramBuilder<T> builder;

    plant_ = builder.template AddSystem<RobobeePlant<T>>();
 ...
 private:
  RobobeePlant<T>* plant_{};
};

//  A robobee as a RigidBodyPlant that is created from a model
// specified in a URDF file.
template<typename T>
class RigidBodyRobobee: public systems::Diagram<T> {
 public:
  RigidBodyRobobee() {
    this->set_name("robobee");
...
//  ge_model_:  GenericRobobee
//  rb_model_:  RigidBodyPlant
class RobobeeTest: public ::testing::Test {
 public:
  RobobeeTest() {
    ge_model_ = std::make_unique<GenericRobobee<double>>();
    rb_model_ = std::make_unique<RigidBodyRobobee<double>>();
 ...
 protected:
  ...
  std::unique_ptr<GenericRobobee<double>> ge_model_;
  std::unique_ptr<RigidBodyRobobee<double>> rb_model_;
...
TEST_F(RobobeeTest, derivatives) {
...
TEST_F(RobobeeTest, drop_from_rest) {
...
TEST_F(RobobeeTest, drop_from_initial_velocity) {
  VectorX<double> x0 = VectorX<double>::Zero(12);
...
TEST_F(RobobeeTest, drop_from_initial_rotation) {
  VectorX<double> x0 = VectorX<double>::Zero(12);
...
// state.
TEST_F(RobobeeTest, drop_from_arbitrary_state) {
  VectorX<double> x0 = VectorX<double>::Zero(12);
  x0 << 1, 2, 3, 4, 5, 6, 1, 2, 3, 4, 5, 6;  // Some initial state.
  PassiveBehaviorTest(x0);
}

TEST_F(RobobeeTest, ToAutoDiff) {
  const RobobeePlant<double> plant;
  EXPECT_TRUE(is_autodiffxd_convertible(plant));
}

TEST_F(RobobeeTest, ToSymbolic) {
  const RobobeePlant<double> plant;
